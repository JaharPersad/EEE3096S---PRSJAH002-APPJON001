...
//	//and it will read 1 byte of data, increment the register address, write another byte and so on
	...
	// Next block was for testing purposes:
	/*
	for( int i=0; i<7;i++){
		printf("%d:",get_time[i]);
	}
...
int epochFromTime(TIME time){
    ...
    for (int i = 0; i<time.month-1;i++){
        numDays = numDays + monthArr[i];
    }
    numDays = numDays + time.dayofmonth;
    uint8_t et = EPOCH_2022;
    return (et+(time.year*365*24*60*60 + numDays*24*60*60 + time.hour*60*60 + time.minutes*60 + time.seconds));
}
